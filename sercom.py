    ...
    def open(self):
        try:
            if not self.is_connected():
                ...
                self.serial_device_connected = True
...
class SerialMonitor:
    def __init__(self):
        ...
        self.serial_device_connected = False
        self.__parser_args()

        # Windows style
        self.output_buffer = TextArea(style="bg:default fg:#ffffff", scrollbar=True)
        self.output_logs = TextArea(
            style="bg:default fg:#ffffff", height=2, read_only=True, scrollbar=True
        )
        self.output_timestamp = TextArea(style="bg:default fg:#ffffff", scrollbar=True)
        self.output_hex = TextArea(style="bg:default fg:#ffffff", scrollbar=True)
        self.input_field = TextArea(
            height=5,
            prompt=self.prompt_char,
            multiline=False,
            focus_on_click=True,
            accept_handler=self.send_input,
            style="bg:default fg:#00c0de",
        )
                ...
                VSplit(
                    [
                        Frame(
                            ...
                            title="Time",
                            width=int(os.get_terminal_size().columns * (2 / 16)),
                        ),
                        Frame(
                            self.output_hex,
                            title="Hex",
                            width=int(os.get_terminal_size().columns * (5 / 16)),
                        ),
                        Frame(self.output_buffer, title=self.prompt_title),
                    ]
                ),
                Frame(self.output_logs, title="Script Logs"),
                self.input_field,
            ]
        )
    ...
    def __monitor_show_data(self, data):
        ...
        if self.configuration["hex"]:
            ...
            formatted_hex = "\n".join(
                [":".join(textwrap.wrap(block, 2)) for block in hex_blocks]
            )
            self.output_hex.text += f"\n{formatted_hex}"
            self.output_hex.buffer.cursor_position = len(self.output_hex.text)

        if self.configuration["timestamp"]:
            time_now = datetime.now().strftime("%H:%M:%S")
            self.output_timestamp.text += "\n" + time_now + "\n" * (row_cr_count - 1)
            self.output_timestamp.buffer.cursor_position = len(self.output_timestamp.text)

        if self.configuration["ascii"]:
            self.output_buffer.text += "\n" + data + "\n" * (row_cr_count - 1)
            self.output_buffer.buffer.cursor_position = len(self.output_buffer.text)
    ...
    def rx_worker(self):
        self.serial_worker = threading.Thread(
            target=self.__rx_serial_worker, daemon=True
        )
